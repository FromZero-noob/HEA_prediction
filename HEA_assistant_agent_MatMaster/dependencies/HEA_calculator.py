...
def HEA_calculate(input,binary_Hmix_data_path):
    ...
    HEA_name = str(input)
    splitter = NameSplit()
    splitter.transform(HEA_name,folds_name=folds_file,expands_name=expands_file)
...
def Rules_check(input_csv):
    ...
    #input = 'D:\\DPTechnology\\HEA_Structure_Prediction\\datasets\\Final_dataset\\_dataset_primal.csv'
    #output_path_2 = 'D:\\DPTechnology\\HEA_Structure_Prediction\\datasets\\old_datasets\\dataset_train_Tm.csv'
    # 1.初始成分
    #data_0 = pd.read_csv(input)
    #data = name_init(input)
    #data.drop_duplicates(data.columns[1:],keep = 'first')

    # 2.计算VEC,delta,Hmix,Smix,Lambda,Omega,Tm
    #data_2 = HEA_calculate(data)
    #data_2['Type']=data_0['Type'].values
    #data_2['Structure']=data_0['Structure'].values

    
    # 3.检查判据
    #data = Rules_check(output_path_2)
    
    # 4. 编码标签列（保存临时文件）
    #data_3=encode_labels(data_2,['Type'])
    #data_3.to_csv('D:\\DPTechnology\\HEA_Structure_Prediction\\datasets\\Final_dataset\\_dataset_train_labeled_type.csv')
    #df = pd.read_csv('D:\\DPTechnology\\HEA_Structure_Prediction\\datasets\\Final_dataset\\_dataset_train_labeled_type.csv')
    #df = df[df['Type'] == 1]
    #df.to_csv('D:\\DPTechnology\\HEA_Structure_Prediction\\datasets\\Final_dataset\\_dataset_train_unlabeled_structure.csv')
    #df = encode_labels(df,['Structure'])
    #df.to_csv('D:\\DPTechnology\\HEA_Structure_Prediction\\datasets\\Final_dataset\\_dataset_train_labeled_structure.csv')
